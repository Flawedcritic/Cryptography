...
def get_keys():
    while True:
        try:
            ...
            key1 = int(input("Enter first key: "))  # Get first key
            key2 = int(input("Enter second key: "))  # Get second key
...
def affine_encrypt():
    ...
    sent = input("What text do you want to encrypt?: ")  # Get text to encrypt
...
# Main function
def main():
    a = enc_or_dec()  # Get user's choice (encrypt or decrypt)
    if a == '1':
        value = affine_encrypt()  # Encrypt if user chooses 1
        print(value)  # Print encrypted text
    elif a == '2':
        value = affine_decrypt()  # Decrypt if user chooses 2
        print(value)  # Print decrypted text
    elif a == 'q':
        print("quitting!")  # Quit if user chooses q
